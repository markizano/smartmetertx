...
Simply use the same `$SMTX_FROM` and `$SMTX_TO` variables described in
[smtx-fetch.md](./smtx-fetch.md) to generate a report on a date range, like so:

```bash
AWS_PROFILE=sns SMTX_FROM=2024-11-11 SMTX_TO=2024-12-11 smtx-report
```
