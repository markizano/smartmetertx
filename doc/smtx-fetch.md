...
If you want to catch up on previous meter reads, you can use environment variables to set the dates
parseable by [dateparser](https://dateparser.readthedocs.io/en/latest/).
...
The TO date where to stop results. This is usually not inclusive, so if you want the target date,
specify the day after that.
...
```cron
2 2 * * * smartmetertx smtx-fetch
```

## Example CRON script

In this script, I have it setup to ensure the script runs as the specified user since I don't like
...
. ~/bin/activate
echo "Running smtx-fetch as smartmetertx:apps"
...
```bash
SMTX_FROM=2024-01-11 SMTX_TO=2024-02-12 smtx-fetch
```
